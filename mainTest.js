...
function timeCheck() {
  const fixtureTimerContainer = document.querySelectorAll(".ipn-Fixture_TimerContainer");
  fixtureTimerContainer.forEach(time => {
    let min = time.textContent.slice(0,2);
    if(+min < 45) {
      time.parentNode.parentNode.style.cssText = "display:none;";
    } else if (+min <= 75){
      time.parentNode.parentNode.style.cssText = "display:flex;";
    } else {
      time.parentNode.parentNode.remove();
      remeveCompCont();
    }
...
function saveDate() {
  ...
  filterWord.forEach(word => {
    $("div.ipn-Competition:contains(" + word + ")").hide();
  });
